...
namespace DAM
{
    ...
    public class FLHookSocket
    {
        ...
        /// <returns>Returns Rep_Group if successfull otherwise ERROR.</returns>

public string CmdReadCharFile(string CharName)
		{
			int i = 0;
			string result;
			try
			{
				this.SendCommand(this.cmdStream, string.Format("readcharfile {0}", CharName), this.unicode);
				while (i < 5000)
				{
					string[] array = new string[5000];
					array[i] = this.ReceiveReply(this.cmdStream, this.unicode);
					if (array[i].StartsWith("l rep_group"))
					{
						int num = i;
